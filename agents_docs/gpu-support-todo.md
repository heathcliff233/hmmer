...
- Exact hit parity currently depends on CPU-compatible checks at the bias-corrected F1 boundary: CPU `p7_bg_FilterScore()` supplies the final bias score for GPU MSV survivors, and CPU `p7_MSVFilter()` can rescue bias-boundary rejects through the optimized CPU SSV shortcut. A narrower CUDA-bias verification attempt was faster but reintroduced `4TM_phosphoesterase` CPU-only hits, so do not remove or narrow this boundary without all-13 profmark parity evidence.
...
- Current correctness-first combined all-13 timing (`benchmark-data/profmark-current/gpu-audit/cpu-bias-msv-rescue-all13/logs/profmark-gpu-summary.tsv`) shows the later-stage path can preserve exact final hit parity while speeding up over CPU: CPU wall 26.82 sec, GPU wall 16.32 sec, aggregate speedup 1.643x, `cpu_only=0`, and `gpu_only=0`.
- Broader 12-query timing outside the prepared all-13 set (`benchmark-data/profmark-current/gpu-audit/broader12-nocompare/logs/profmark-gpu-summary.tsv`) also preserved exact final hit parity with the combined later-stage flags and recorded a smaller aggregate speedup: CPU wall 7.78 sec, GPU wall 7.05 sec, aggregate speedup 1.104x, `cpu_only=0`, and `gpu_only=0`. This sample is dominated by shorter/fast CPU queries, so it is evidence that the path can remain positive outside the all-13 set, not proof that every profile class benefits.
- On the same broader 12-query sample, the accepted MSV/bias GPU baseline (`benchmark-data/profmark-current/gpu-audit/broader12-msvbias-baseline/logs/profmark-gpu-summary.tsv`) recorded GPU wall 7.88 sec. The combined later-stage path therefore saved 0.83 sec aggregate GPU wall and cut CPU survivor continuation from 2.628 sec to 0.597 sec, but `2OG-FeII_Oxy_3` and `2TM` regressed slightly. Future default policy should auto-gate by profile/candidate shape rather than enabling every later-stage kernel unconditionally.
- Raising `--gpu-fwd-min-seqs` to 32 on that broader sample (`benchmark-data/profmark-current/gpu-audit/broader12-fwdmin32/logs/profmark-gpu-summary.tsv`) reduced CUDA Forward batches from 202 to 81 but worsened aggregate GPU wall time from 7.05 sec to 7.60 sec. Launch count alone is not a sufficient auto-gating rule.
- The matching broader 12-query compare run (`benchmark-data/profmark-current/gpu-audit/broader12-compare/logs/profmark-gpu-summary.tsv`) produced zero `CUDAVIT` and zero `CUDAFWD` diagnostics, preserved final hit parity, and kept domain-decoding input maxima bounded at `max_mocc=0.000005`, `max_btot=0.000011`, and `max_etot=0.000010` despite 57 raw `CUDAFB` scale/xmx diagnostics. Compare mode reruns CPU checks and must not be used as the speed claim.
