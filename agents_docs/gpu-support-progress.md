...
- Viterbi is the next bounded candidate, but it needs a CUDA-specific Viterbi profile or a carefully unpacked optimized profile, plus survivor compaction so the GPU does not score the full database. Forward/Backward should wait until the Viterbi filter boundary is understood because ForwardParser output feeds BackwardParser and domain definition.
