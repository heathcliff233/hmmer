...
- Forward/Backward is partially suitable, but not as a simple isolated score offload unless the implementation explicitly handles the CPU matrix-state dependency. A narrow score prefilter could be benchmarked after Viterbi survivors, but it must be compared against the current MSV+bias baseline and must include the cost of rerunning CPU Forward for F3 survivors.
