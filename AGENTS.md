...
- Current nucleotide benchmark snapshot: chr22/hg38 target, query_medium (M=501), RTX 4090, 4 CPU threads. After sorting GPU SSV windows before the first long-target extend/merge, and sorting GPU scanning-Viterbi seeds by parent window before local-coordinate merge, default overlap `.nucdb` is hit-count clean versus CPU (`648` output rows in the smoke run). Same-mode rerun on 2026-05-10: CPU-4 FASTA HMMER elapsed 1.87s (`/usr/bin/time` 1.89s); default GPU fast `.nucdb` HMMER elapsed 1.40s (`/usr/bin/time` 1.75s), with GPU timing total 1.297s. GPU workers are still the largest bucket (1.024s), almost entirely CPU domain workflow (1.013s); CPU Forward/Backward continuation is zero because GPU Forward/Backward parser handoff is default. The old 1.91s versus 2.109s discrepancy was apples-to-oranges and stale: targeted handoff/no-overlap runs were compared to an older quick-benchmark overlap run before the GPU window-order fixes.
