...
- Current nucleotide benchmark snapshot: chr22/hg38 target, query_medium (M=501), RTX 4090, 4 CPU threads. After sorting GPU SSV windows before the first long-target extend/merge, sorting GPU scanning-Viterbi seeds by parent window before local-coordinate merge, and removing the extra single-score GPU Viterbi prefilter, default overlap `.nucdb` is hit-count clean versus CPU (`648` main-output hit lines in the smoke run; strict `--tblout` CPU FASTA vs GPU fast `.nucdb` rows both `215` with no diff). Same-mode speed script rerun on 2026-05-10: CPU-4 FASTA 1.669s; default GPU fast `.nucdb` 1.232s. Focused repeat runs showed CPU-4 FASTA wall 1.77s and GPU fast `.nucdb` wall 1.53-1.86s, with GPU timing totals 1.141-1.319s. GPU workers are still the largest bucket, almost entirely CPU domain workflow (~0.79s in focused repeats); CPU Forward/Backward continuation is zero because GPU Forward/Backward parser handoff is default.
