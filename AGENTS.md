...
- Forward/Backward is only partially suitable as a simple GPU target: `p7_ForwardParser()` supplies both the F3 score and `P7_OMX` state needed by Backward/domain. The all-13 run bounds a perfect Forward score prefilter at about 1.97 sec saved from a 14.66 sec GPU run after CPU Forward reruns for F3 survivors. Backward, domain definition, null2, hit storage, thresholding, and output should stay CPU-side for now.
