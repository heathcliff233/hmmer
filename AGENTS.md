...
- Current profmark runs show real end-to-end speedups on representative protein queries. The current correctness-first all-13 run in `benchmark-data/profmark-current/gpu-audit/cpu-bias-msv-rescue-all13/` with `--gpu-vit-prefilter --gpu-fwd-prefilter --gpu-fb-parser` recorded CPU wall 26.82 sec, GPU wall 16.32 sec, aggregate speedup 1.643x, median speedup 1.508x, `cpu_only=0`, and `gpu_only=0`. Treat `benchmark-data/profmark-current` as the real benchmark, not tutorial-sized inputs.
