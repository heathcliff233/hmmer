...
- Remaining GPU costs are mainly CPU survivor continuation after bias. Viterbi is still a logical bottleneck, but the first candidate-only CUDA prototype was rejected as slower and not parity-safe.
- Forward/Backward is only partially suitable as a simple GPU target: `p7_ForwardParser()` supplies both the F3 score and `P7_OMX` state needed by Backward/domain. The accepted experimental path is still the short-profile Forward score prefilter with CPU Forward rerun for F3 survivors. A newer `--gpu-fb-parser` handoff exists as scaffolding for parser `xmx` state and now batches F3 survivors per search chunk. Its short-profile Backward parser kernel is block-parallel and fast on the checked profmark `14-3-3` case, but it is not a broad speed feature yet: parser Forward is still scalar per survivor, CPU work still gates/orchestrates the handoff, and compare mode still reports `p7X_SCALE` row differences even when Forward/Backward scores and final hit names match. Backward, domain definition, null2, hit storage, thresholding, and output should stay CPU-side for accepted/default paths.
