...
- Current profmark runs show real end-to-end speedups on representative protein queries. The current-build all-13 MSV/bias baseline in `benchmark-data/profmark-current/gpu-audit/baseline-current-all13-rerun/` recorded CPU wall 37.07 sec, GPU wall 20.96 sec, aggregate speedup 1.769x, four CPU-only hits from known SSV/MSV sensitivity gaps, and no GPU-only hits. Treat `benchmark-data/profmark-current` as the real benchmark, not tutorial-sized inputs.
