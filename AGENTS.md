...
- Current nucleotide benchmark snapshot: chr22/hg38 target, RTX 4090, 4 CPU threads. After sorting GPU SSV windows before the first long-target extend/merge, sorting GPU scanning-Viterbi seeds by parent window before local-coordinate merge, removing the extra single-score GPU Viterbi prefilter, and changing GPU CPU-continuation workers from static equal-window slices to dynamic survivor-window scheduling, default overlap `.nucdb` is hit-count clean versus CPU in the smoke script (`MADE1`, `query_short`, `query_medium`: 4/4 parity checks passed). `test-speed/x-nhmmer-gpu-bench` now defaults to a combined multi-query run over all three samples so CUDA engine creation and resident `.nucdb` upload are amortized. Latest combined run on 2026-05-10 after dynamic scheduling: CPU-1 8.625s / 1476 hits, CPU-4 2.570s / 1476 hits, GPU FASTA 2.094s / 1476 hits, GPU no-overlap `.nucdb` 1.786s / 1476 hits, GPU overlap `.nucdb` 1.640s / 1476 hits. Per-query mode remains available with `NHMMER_GPU_BENCH_MODE=per-query`; query_medium GPU-16 now matches CPU-16 domain wall time in focused repeats.
- GPU timing output now includes search-stage timing plus outside-search accounting. For `nhmmer --gpu`, stderr reports GPU search stages, GPU loop wall, `.nucdb` reconstruction, pre-search setup, post-loop cleanup, post-search/report time, query outside-search time, query elapsed, CUDA engine creation/reset/destroy, shared `.nucdb` open/upload, process outside-search time, and total process elapsed. CUDA event timings remain device-side sub-buckets. CPU pipeline `Stage *` totals are still summed across worker threads, but `HMMER_NHMMER_CPU_WALL_TRACE=1` prints a max-across-worker CPU wall-stage estimate for CPU-only threaded `nhmmer`.
