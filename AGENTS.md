...
- Current nucleotide benchmark snapshot: chr22/hg38 target, RTX 4090, 4 CPU threads. After sorting GPU SSV windows before the first long-target extend/merge, sorting GPU scanning-Viterbi seeds by parent window before local-coordinate merge, and removing the extra single-score GPU Viterbi prefilter, default overlap `.nucdb` is hit-count clean versus CPU in the smoke script (`MADE1`, `query_short`, `query_medium`: 4/4 parity checks passed). `test-speed/x-nhmmer-gpu-bench` now defaults to a combined multi-query run over all three samples so CUDA engine creation and resident `.nucdb` upload are amortized. Latest combined run on 2026-05-10: CPU-1 8.333s / 1476 hits, CPU-4 2.297s / 1476 hits, GPU FASTA 2.982s / 1476 hits, GPU no-overlap `.nucdb` 2.022s / 1476 hits, GPU overlap `.nucdb` 1.798s / 1476 hits. Per-query mode remains available with `NHMMER_GPU_BENCH_MODE=per-query`; query_medium focused timings still show CPU domain workflow as the dominant bucket.
- GPU timing output now includes search-stage timing plus outside-search accounting. For `nhmmer --gpu`, stderr reports GPU search stages, GPU loop wall, `.nucdb` reconstruction, pre-search setup, post-loop cleanup, post-search/report time, query outside-search time, query elapsed, CUDA engine creation/reset/destroy, shared `.nucdb` open/upload, process outside-search time, and total process elapsed. CUDA event timings remain device-side sub-buckets; CPU pipeline `Stage *` totals are still summed across worker threads and are not directly additive with wall-clock buckets.
