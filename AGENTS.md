...
- Current nucleotide benchmark snapshot: chr22/hg38 target, RTX 4090, 16 CPU threads. After sorting GPU SSV windows before the first long-target extend/merge, moving nucleotide batch F1 gating to a B1-scaled CUDA gate with order-preserving survivor compaction, sorting GPU scanning-Viterbi seeds by parent window before local-coordinate merge, removing the extra single-score GPU Viterbi prefilter, changing GPU CPU-continuation workers from static equal-window slices to dynamic survivor-window scheduling, remapping scanning Viterbi to four 8-lane nucleotide DP groups per physical warp, reusing engine-owned Viterbi long-target CUDA streams, keeping FASTA and `.nucdb` Forward parser xmx resident through GPU F3 gating/survivor compaction/Backward, and caching reconstructed `.nucdb` host strands across queries, default overlap `.nucdb` is hit-count clean versus CPU in the smoke script (`MADE1`, `query_short`, `query_medium`: 4/4 parity checks passed with 16 threads). `test-speed/x-nhmmer-gpu-bench` now defaults to CPU/GPU `--cpu 16` and a combined multi-query run over all three samples so CUDA engine creation and resident `.nucdb` upload are amortized. Latest combined run on 2026-05-10 after the nucleotide GPU F1 gate: CPU-1 8.823s / 1476 hits, CPU-16 1.090s / 1476 hits, GPU-16 FASTA 2.380s / 1476 hits, GPU-16 no-overlap `.nucdb` 1.568s / 1476 hits, GPU-16 overlap `.nucdb` 1.202s / 1476 hits. A direct combined overlap `.nucdb` timing run showed batch filter buckets of `0.015s`, `0.009s`, and `0.036s` for MADE1/query_short/query_medium, reconstruction only on the first query (`0.078s`, then `0.000s` and `0.000s`), process elapsed `1.649s`, and GPU loop wall total `0.979s`. Per-query mode remains available with `NHMMER_GPU_BENCH_MODE=per-query`; `NHMMER_GPU_BENCH_CPU=N` overrides the default thread count.
