...
if (! -x "$builddir/easel/miniapps/easel")   { die "FAIL: didn't find easel miniapps binary in $builddir/easel/miniapps";  }
...
foreach $trial (1..5)
{
	...
	# random sequences
	`$builddir/easel/miniapps/easel synth -o $tmppfx.fa amino $n 50`;
...
sub hmmalign_msa_integrity_check
{
    ...
    `$builddir/easel/miniapps/easel reformat -u fasta $tmppfx.sto > $tmppfx.fa1 2>/dev/null`;
    if ($? != 0) { die "FAIL: first esl-reformat failed"; }
    
    `$builddir/easel/miniapps/easel reformat -u fasta $fafile    > $tmppfx.fa2 2>/dev/null`;
...
sub hmmsearch_msa_integrity_check
{
    ...
    # Grep out name/desc lines because they'll differ: easel sfetch adds descline.
    `$builddir/easel/miniapps/easel reformat -u fasta $tmppfx.sto | grep -v "^>" > $tmppfx.fa1 2>/dev/null`;
    if ($? != 0) { die "FAIL: first easel reformat failed"; }
    
    `$builddir/easel/miniapps/easel reformat -u fasta $tmppfx.fa3 | grep -v "^>" > $tmppfx.fa2 2>/dev/null`;
