 ...
 */
  ...
  char       *ffile256;    // .h3f file: binary vectorized profiles, MSV filter part only, 256-bit version
  char       *ffile512;    // .h3f file: binary vectorized profiles, MSV filter part only, 512-bit version
...
main(int argc, char **argv)
{
  ...
  hfp->ffp = dbf->ffp;
  hfp->ffp256 = dbf->ffp256;
  hfp->ffp512 = dbf->ffp512;
  hfp->pfp = dbf->pfp;
  ...
  while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) == eslOK)
	...
	p7_bg_SetLength(bg, 400);
      ...
      if ((om->offs[p7_FOFFSET256] = ftello(dbf->ffp256)) == -1) ESL_XFAIL(eslESYS, errbuf, "Failed to ftello() current disk position of MSV db file (256-bit SIMD)");
      if ((om->offs[p7_FOFFSET512] = ftello(dbf->ffp512)) == -1) ESL_XFAIL(eslESYS, errbuf, "Failed to ftello() current disk position of MSV512 db file");
...
open_dbfiles(ESL_GETOPTS *go, char *basename)
{
  ...
  if ( (status = esl_sprintf(&(dbf->ffile256),   "%s.h3f256", basename)) != eslOK) ESL_XFAIL(status, errbuf, "esl_sprintf() failed");
  if ( (status = esl_sprintf(&(dbf->ffile512),   "%s.h3f512", basename)) != eslOK) ESL_XFAIL(status, errbuf, "esl_sprintf() failed");
  ...
  if (! allow_overwrite && esl_FileExists(dbf->ffile256))   ESL_XFAIL(eslEOVERWRITE, errbuf, "Binary MSV filter file %s already exists\nDelete old hmmpress indices first", dbf->ffile256); 
  if (! allow_overwrite && esl_FileExists(dbf->ffile512))   ESL_XFAIL(eslEOVERWRITE, errbuf, "Binary MSV filter file %s already exists\nDelete old hmmpress indices first", dbf->ffile512); 
  ...
  if ((dbf->ffp256 = fopen(dbf->ffile256, "wb")) == NULL)  ESL_XFAIL(eslEWRITE, errbuf, "Failed to open binary MSV filter file %s for writing", dbf->ffile256); 
  if ((dbf->ffp512 = fopen(dbf->ffile512, "wb")) == NULL)  ESL_XFAIL(eslEWRITE, errbuf, "Failed to open binary MSV filter file %s for writing", dbf->ffile512); 
  ...
  if (dbf)
    {
      ...
      if (dbf->ffp256)  fclose(dbf->ffp256);
      if (dbf->ffp512)  fclose(dbf->ffp512);
      ...
      if (status != eslOK) 
        {
          ...
          if (esl_FileExists(dbf->ffile256))   remove(dbf->ffile256);
          if (esl_FileExists(dbf->ffile512))   remove(dbf->ffile512);
      ...
      if (dbf->ffile256)   free(dbf->ffile256);
      if (dbf->ffile512)   free(dbf->ffile512);
