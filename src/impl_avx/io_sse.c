...
int
p7_oprofile_Write_sse(P7_HMMFILE *hfp, P7_OPROFILE *om)
{
  int Q4   = p7O_NQF(om->M);
  int Q8   = p7O_NQW(om->M);
...
for (x = 0; x < om->abc->Kp; x++){
  ...
  for (int t = 0; t <(p7O_NTRANS -1); t++){ /* this loop of 7 transitions depends on the order in p7o_tsc_e */
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
      int vector = ((hmmpos%Q8) * (p7O_NTRANS-1)) + t;
      int within_vector = hmmpos/Q8;  
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
      ((uint16_t *)buffer)[(t * om->M)+hmmpos] = short_pointer[index];
    }
  }
  // and the DDs
  for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
    int vector = (hmmpos % Q8) + (Q8 * (p7O_NTRANS -1));
      int within_vector = hmmpos /Q8;
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
  ...
  for (x = 0; x < om->abc->Kp; x++){
    short_pointer = (uint16_t *) om->rwv[x];
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
      int vector = hmmpos % Q8;
      int within_vector = hmmpos /Q8;
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
  ...
  float *float_pointer;
  float_pointer = (float *)om->tfv;
  // everything but the DDs
  for (int t = 0; t <(p7O_NTRANS -1); t++){ /* this loop of 7 transitions depends on the order in p7o_tsc_e */
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
      int vector = ((hmmpos%Q4) * (p7O_NTRANS-1)) + t;
      int within_vector = hmmpos/Q4;  
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
      ((float *)buffer)[(t * om->M)+hmmpos] = float_pointer[index];
    }
  }
  // and the DDs
  for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
    int vector = (hmmpos % Q4) + (Q4 * (p7O_NTRANS -1));
      int within_vector = hmmpos /Q4;
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
  ...
  for (x = 0; x < om->abc->Kp; x++){
    float_pointer = (float *) om->rfv[x];
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
      int vector = hmmpos % Q4;
      int within_vector = hmmpos /Q4;
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
...
p7_oprofile_ReadRest_sse(P7_HMMFILE *hfp, P7_OPROFILE *om)
{
  ...
  Q8  = p7O_NQW(om->M);
  char *buffer;
  ESL_ALLOC(buffer, sizeof(__m128i) * Q4 * p7O_NTRANS); // big enough for any row
  ...
  for (int t = p7O_BM; t <= p7O_II; t++){ /* this loop of 7 transitions depends on the order in p7o_tsc_e */
    ...
    for(int padding = om->M; padding < (Q8 * (sizeof(__m128i)/sizeof(uint16_t))); padding++){
      int vector = ((padding%Q8) * (p7O_NTRANS-1)) + t;
      int within_vector = padding/Q8;  
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
  ...
  for(int padding = om->M; padding < (Q8 * (sizeof(__m128i)/sizeof(uint16_t))); padding++){
      int vector = (padding % Q8) + (Q8 * (p7O_NTRANS -1));
      int within_vector = padding /Q8;
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
      short_pointer[index] = -32768;
  }
  ...
  for (x = 0; x < om->abc->Kp; x++){
    ...
    for(int padding = hmmpos; padding < (Q8 * (sizeof(__m128i)/sizeof(uint16_t))); padding++){
      int vector = padding % Q8;
      int within_vector = padding /Q8;
      int index = (vector * (sizeof(__m128i)/sizeof(uint16_t)))+within_vector;
 ...
 if (! fread((char *) buffer, sizeof(float),  p7O_NTRANS*om->M, hfp->pfp)) ESL_XFAIL(eslEFORMAT, hfp->rr_errbuf, "failed to read <tf>, fwd/back transitions");
  float *float_pointer = (float *) om->tfv;
  // everything but the DDs
  for (int t = p7O_BM; t <= p7O_II; t++){ /* this loop of 7 transitions depends on the order in p7o_tsc_e */
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
      int vector = ((hmmpos%Q4) * (p7O_NTRANS-1)) + t;
    ...
    for(int padding = om->M; padding < (Q4 * (sizeof(__m128i)/sizeof(float))); padding++){
      int vector = ((padding%Q4) * (p7O_NTRANS-1)) + t;
      int within_vector = padding/Q4;  
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
  ...
  for(int padding = om->M; padding < (Q4 * (sizeof(__m128i)/sizeof(float))); padding++){
      int vector = (padding % Q4) + (Q4 * (p7O_NTRANS -1));
      int within_vector = padding /Q4;
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
      float_pointer[index] = 0;
  }
  ...
  for (x = 0; x < om->abc->Kp; x++){
    ...
    for(int padding = hmmpos; padding < (Q4 * (sizeof(__m128i)/sizeof(float))); padding++){
      int vector = padding % Q4;
      int within_vector = padding /Q4;
      int index = (vector * (sizeof(__m128i)/sizeof(float)))+within_vector;
