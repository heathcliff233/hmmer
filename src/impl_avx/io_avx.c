...
int
p7_oprofile_Write_avx(P7_HMMFILE *hfp, P7_OPROFILE *om)
{
...
for (x = 0; x < om->abc->Kp; x++){
  rbv_scalar = (uint8_t *) om->sbv_avx[x];
  ...
  for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
    ...
    int index = (vector * sizeof(__m256i))+within_vector;
...
for (x = 0; x < om->abc->Kp; x++){
    rbv_scalar = (uint8_t *) om->rbv_avx[x];
  ...
  float *float_pointer;
  float_pointer = (float *)om->tfv_avx;
  // everything but the DDs
  for (int t = 0; t <(p7O_NTRANS -1); t++){ /* this loop of 7 transitions depends on the order in p7o_tsc_e */
    for(int hmmpos = 0; hmmpos < om->M; hmmpos++){
...
p7_oprofile_ReadMSV_avx(P7_HMMFILE *hfp, ESL_ALPHABET **byp_abc, P7_OPROFILE **ret_om)
{
  ...
  if (hfp->ffp256 == NULL) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "no MSV profile file; hmmpress probably wasn't run");
  if (feof(hfp->ffp256))   { status = eslEOF; goto ERROR; }	/* normal EOF: no more profiles */
  
  /* keep track of the starting offset of the MSV model */
  roff = ftello(hfp->ffp256);

  if (! fread( (char *) &magic,     sizeof(uint32_t), 1, hfp->ffp256)) { status = eslEOF; goto ERROR; }
  ...
  if (! fread( (char *) &M,         sizeof(int),      1, hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read model size M");
  if (! fread( (char *) &alphatype, sizeof(int),      1, hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read alphabet type");  
  ...
  om->M = M;
  om->roff = roff;
  if (! fread((char *) &n,               sizeof(int),     1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read name length");
  ESL_ALLOC(om->name, sizeof(char) * (n+1));
  if (! fread((char *) om->name,         sizeof(char),    n+1,         hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read name");

  if (! fread((char *) &(om->max_length),sizeof(int),     1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read max_length");
  if (! fread((char *) &(om->tbm_b),     sizeof(uint8_t), 1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read tbm");
  if (! fread((char *) &(om->tec_b),     sizeof(uint8_t), 1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read tec");
  if (! fread((char *) &(om->tjb_b),     sizeof(uint8_t), 1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read tjb");
  if (! fread((char *) &(om->scale_b),   sizeof(float),   1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read scale");
  if (! fread((char *) &(om->base_b),    sizeof(uint8_t), 1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read base");
  if (! fread((char *) &(om->bias_b),    sizeof(uint8_t), 1,           hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read bias");

  for (x = 0; x < abc->Kp; x++){
    if (! fread((char *) om->sbv_avx[x],     sizeof(__m256i), Q16x_AVX,        hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read ssv scores at %d [residue %c]", x, abc->sym[x]); 
  //  p7_restripe_byte((char *) om->sbv[x], (char*) om->sbv_avx[x], padded_byte_vector_length, 128, 256, 255);
  }
  for (x = 0; x < abc->Kp; x++){
    if (! fread((char *) om->rbv_avx[x],     sizeof(__m256i), Q16_AVX,         hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read msv scores at %d [residue %c]", x, abc->sym[x]);
    //p7_restripe_byte((char *) om->rbv[x], (char *) om->rbv_avx[x], byte_vector_length, 128, 256, 255);
  }
  if (! fread((char *) om->evparam,      sizeof(float),   p7_NEVPARAM, hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read stat params");
  if (! fread((char *) om->offs,         sizeof(off_t),   p7_NOFFSETS, hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read hmmpfam offsets");
  if (! fread((char *) om->compo,        sizeof(float),   p7_MAXABET,  hfp->ffp256)) ESL_XFAIL(eslEFORMAT, hfp->errbuf, "failed to read model composition");

  /* record ends with magic sentinel, for detecting binary file corruption */
  if (! fread( (char *) &magic,     sizeof(uint32_t), 1, hfp->ffp256))  ESL_XFAIL(eslEFORMAT, hfp->errbuf, "no sentinel magic: .h3f file corrupted?");
...
#endif //ifdef eslENABLE_AVX
#ifndef eslENABLE_AVX // stubs for compilers that can't handle AVX
int p7_oprofile_Write_avx(P7_HMMFILE *hfp, P7_OPROFILE *om){
  return eslEUNSUPPORTEDISA;
}

int p7_oprofile_ReadMSV_avx(P7_HMMFILE *hfp, ESL_ALPHABET **byp_abc, P7_OPROFILE **ret_om){
    return eslEUNSUPPORTEDISA;
}

int p7_oprofile_ReadRest_avx(P7_HMMFILE *hfp, P7_OPROFILE *om) {
    return eslEUNSUPPORTEDISA;
}
#endif