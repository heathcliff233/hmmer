...
extern "C" int
p7_cuda_MSVFilterResident(P7_CUDA_ENGINE *engine, const P7_CUDA_MSVPROFILE *cuom,
                           ...
                           char *errbuf, int errbuf_size)
{
  ...
  if (shmem > 96 * 1024) {
    if (errbuf && errbuf_size > 0) snprintf(errbuf, errbuf_size, "CUDA MSV profile M=%d exceeds shared-memory limit", cuom->M);
    return eslERANGE;
  }
  ...
  cudaEventRecord(k0);
  cuda_msv_batch_kernel<<<nseq, P7_CUDA_MSV_BLOCK_THREADS, shmem>>>(
    ...
    cuom->base_b, cuom->bias_b,
    engine->d_raw, engine->d_overflow);
  if ((status = cuda_status(cudaGetLastError(), errbuf, errbuf_size, "cuda_msv_batch_kernel(resident) launch")) != eslOK) goto CUDA_ERROR;
  cudaEventRecord(k1);
  ht0 = host_seconds();
  cudaEventSynchronize(k1);
  engine->stats.host_sync_seconds += host_seconds() - ht0;
